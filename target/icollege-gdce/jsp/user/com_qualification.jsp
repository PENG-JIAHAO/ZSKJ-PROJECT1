...
<%@ page import="com.isscollege.gdce.domain.OperateRecord" %>
...
$(function () {
	...
	function message(companyId){
		$("#reviewAdvicetTable tr:not(:first)").empty();
        $.ajax({
            type:"get",
            url: "${context}/review/findComReviewAdvice?companyId="+companyId,
            ...
            success: function(result) {
                ...
                jQuery.each(JSON.parse(jsonData), function(i,item){
                     tr='<td style="text-align: center">'+item.createDate+'</td>'+
                        '<td>'+item.comment+'</td>'+
                    $("#reviewAdvicetTable").append('<tr>'+tr+'</tr>');
                });

				$("#reviewAdvicetTable").tab("show");
            }
        });
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-header">
					...
					<h4 class="modal-title" id="mmyModalLabel">错误</h4>
				</div>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button class="close" aria-hidden="true" type="button"
						data-dismiss="modal">×</button>
					<h4 class="modal-title" id="myModalLabell">审核建议</h4>
				</div>
					<c:choose>
						...
						<c:otherwise>
							<table class="table table-hover" id="reviewAdvicetTable">
								<tr>
								...
								<td align="left">
									...
									<div class="row" style="height: 50px;">
										<div class="col-md-3"  float="left">
											<a class="btn btn-primary" onclick="message('${company.getCompanyId() }')" >审核信息</a>
										</div>
									</div>
								</td>
							</tr>
